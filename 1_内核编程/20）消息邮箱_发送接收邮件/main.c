...
static void send_entry(void *parameter)
{
    rt_mb_send(dynamic_mb, (rt_ubase_t) &buf);
    rt_mb_send(dynamic_mb, (rt_ubase_t) &buf1);
    rt_mb_urgent(dynamic_mb, (rt_ubase_t) &buf2);
}
...
static void recv_entry(void *parameter)
{
    ...
    while (1)
    {
        ...
        if (rt_mb_recv(dynamic_mb, (rt_ubase_t *)&str, RT_WAITING_FOREVER) == RT_EOK)
        {
            rt_kprintf("\rrthread1: get a mail from mailbox, the content:%c\n", str[0]);
        }
    }
}
