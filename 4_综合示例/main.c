...
#define MQTT_SUBTOPIC "/mqtt/HeBei/sub/"
#define MQTT_PUBTOPIC "/mqtt/HeBei/pub/"
...
void led_thread_entry(void *parameter)
{
    ...
    while (1)
    {
        ...
        {
            // 判断消息内容并执行相应操作
            if (strcmp(messagedate, "led_on") == 0)
            {
                rt_pin_write(PIN_LED_R, PIN_LOW);
                rt_kprintf("Led on\r\n");
            }
            else if (strcmp(messagedate, "led_off") == 0)
            {
...
int main(void)
{
    ...
    rt_wlan_connect("nubia", "ws051000.");
...
static void mq_start(void)
{
    ...
    /* 配置 MQTT 文本参数 */
    {
        ...
        rt_snprintf(sup_pub_topic, sizeof(sup_pub_topic), "%s%s", MQTT_SUBTOPIC, cid);
        rt_snprintf(pub_topic, sizeof(pub_topic), "%s%s", MQTT_PUBTOPIC, cid);
        /* 配置连接参数 */
